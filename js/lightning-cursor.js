...
// lightning-cursor.js (NO ES MODULE)
// Expose: window.initLightningCursor(options)

window.initLightningCursor = function initLightningCursor(options) {
  const opt = options || {};

  // Chỉ chạy trên thiết bị có chuột (không chạy trên touch)
  if (!window.matchMedia || !window.matchMedia("(pointer: fine)").matches) return;

  // Nếu đã init rồi thì bỏ qua (tránh tạo nhiều canvas)
  if (window.__lightningCursorInited) return;
  window.__lightningCursorInited = true;

  const color = opt.color || "255, 172, 120";     // RGB
  const trailFade = Number.isFinite(opt.trailFade) ? opt.trailFade : 0.18; // 0.10..0.30
  const zIndex = Number.isFinite(opt.zIndex) ? opt.zIndex : 9999;

  const canvas = document.createElement("canvas");
  canvas.className = "cursor-fx-canvas";

  // đảm bảo canvas nằm trên cùng nhưng không chặn click
  ...
  function tick() {
    ...
    // xóa dần nét cũ, KHÔNG phủ nền web
    ctx.globalCompositeOperation = "destination-out";
  ...
  window.destroyLightningCursor = function destroyLightningCursor() {
    cancelAnimationFrame(rafId);
    window.removeEventListener("resize", resize);
    window.removeEventListener("mousemove", onMove);
    window.removeEventListener("mouseleave", onLeave);
    canvas.remove();
    window.__lightningCursorInited = false;
    delete window.destroyLightningCursor;
  };
};
